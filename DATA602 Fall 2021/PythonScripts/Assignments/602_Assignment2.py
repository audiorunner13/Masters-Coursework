...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    # 5. The samples list will contain every 5th reading from the final temperatures list.
    cnt = 4
    i = 0
...
def exercise07(n):
    ...
    while curr < len(n):
        # Check to see if curr == (list lengeth - 1) to account for index 0 and if curr == next then there are no dupes
        if curr == len(n) - 1:
            return False
        # Increment next by one to compare the current value to the next value
        next = curr + 1
        # if the current value is equal to the next value then there is a dupe return true     
        if n[curr] == n[next]:
            return True
...
# def exercise08(s):
    # This function receives a string. The string should be casted to an int and then a float and 
...
def exercise09():
    ...
    # append first dog pic to the list dogs
    dogs.append(str(dog_media.content))
    
    # start while loop at 2
    n = 2
