...
have texted while driving within the past 30 days?
    ...
    Answer: 
```{r text while driving}
(yrbss %>% group_by(text_while_driving_30d) %>% summarise(count_cats = n()))
```
...
```{r no helmet}
head(no_helmet <- yrbss %>%
  filter(helmet_12m == "never"),10)
```

```{r count rows no helmet}
(no_helmet_count <- count(no_helmet))
```
```{r count no helmet and text}
(count_nh_30d_text <- (no_helmet %>% filter(text_while_driving_30d == 30) %>% summarise(count_cats = n())))
```
      Answer:
```{r prop no helmet and 30d text}
(prop_30dtxt_2_nohelmet <- round(count_nh_30d_text/no_helmet_count, 3))
```
...
```{r indicator-texting}
(no_helmet <- no_helmet %>% select(helmet_12m,text_while_driving_30d) %>%
...
1.  What is the margin of error for the estimate of the proportion of non-helmet 
    ...
    this survey?
      ...
      Answer:
$$
ME = 1.96\times SE = 1.96\times\sqrt{\hat{p}(1-\hat{p})/n} \,.
$$
    $$ 1.96\times\sqrt{.066(1-.066)/463}$$
```{r}
(ME_30dtxt_2_nohelmet <- round(1.96 * (sqrt((prop_30dtxt_2_nohelmet * (1 - prop_30dtxt_2_nohelmet))/count_nh_30d_text)),4))
```
1.  Using the `infer` package, calculate confidence intervals for two other 
    ...
    genders first, and then use these data sets to construct the confidence intervals.
...
```
    ...
    The counts within each category for the amount of days male students did strength
    within the past 7 days.
```{r strength training days male}
(yrbss_male %>% group_by(strength_training_7d) %>% summarise(count_actv_cats = n()))
```

      What is the proportion of males who are physically active and have done strength training in 
      the past 7 days and ?

      Male students that are physically active.
```{r physically active males}
active_males <- yrbss_male %>%
  filter(physically_active_7d > 0)
```

      The count of physically active males.
```{r count physically active males}
(num_phys_actv_count <- count(active_males))
```
      
      The count of physically active males that did strenght training in the last 7 days.
```{r phys actv males who strgth trn in last 7 d}
(count_phact_7d_strtrn <- (active_males %>% filter(strength_training_7d == 7) %>% summarise(count_pa_cats = n())))
```
It may be easier to calculate the proportion if you create a new variable
that specifies whether the individual has strength training every day while being physically active over the past 7 days or not. We will call this variable `str_trn_ind`.

```{r indicator-str_trng}
(active_males <- active_males %>% select(physically_active_7d,strength_training_7d) %>%
  mutate(str_trn_ind = ifelse(strength_training_7d == "7", "yes", "no")))
```


    Calcualte the observed statistic.

```{r Calculate the p_hat}
(p_hat <- round(active_males %>%
  specify(response = str_trn_ind, success = "yes") %>%
  calculate(stat = "prop"),3))
```

      Calculate the Confidence Interval
```{r phys_act-str_trn-ci}
round(active_males %>%
  specify(response = str_trn_ind, success = "yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
...
ME = 1.96\times SE = 1.96\times\sqrt{\hat{p}(1-\hat{p})/n} \,.
$$
    $$ ME = 1.96\times\sqrt{.258(1-.258)/1441}$$
```{r calc Margin of Error}
(ME_phact_7d_strtrng <- round(1.96 * (sqrt((p_hat * (1 - p_hat))/count_nh_30d_text)),4))
```
...
1.  Describe the relationship between `p` and `me`. Include the margin of
    ...
    Answer:  The relationship of 'p' and 'me' is one of a normal distribution in that as population proportion
             'p' increases from 0 the margin of error increases until it reaches a maximum of .03 at .50 of the
             population distribution and then decreases again as it approaches 1.0.
...
1.  Describe the sampling distribution of sample proportions at $n = 300$ and 
    $p = 0.1$. Be sure to note the center, spread, and shape.
      ...
      Answer:  I appears to have close to a normal distribution with the center is at .10, an even spread
...
1.  Keep $n$ constant and change $p$. How does the shape, center, and spread 
    ...
    Answer:  The shape of the distribution gets closer to normal as the population proportion approached
             .50 as the plot above shows.
    
1.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?
    
    Answer:  As the sample size increases it appears that although the distribution is close to normal the 
             spread seems to be more narrow.
