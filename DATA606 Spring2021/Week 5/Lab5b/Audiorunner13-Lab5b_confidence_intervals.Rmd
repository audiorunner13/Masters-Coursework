...
```
    ...
    Answer:  58% of the adults in the sample believe that climate chnage affects their local community.

1.  Would you expect another student's sample proportion to be identical to yours? 
    Would you expect it to be similar? Why or why not?
    
    Answer:  I do not think that another student's sample proportion would be identical but I would                  expect it to be similar due to the size of the sample.
...
1.  Does your confidence interval capture the true population proportion of US adults 
    ...
    Answer: Yes, the true population proportion is captured in the confidence interval.
...
1.  Each student should have gotten a slightly different confidence interval. What 
    ...
    Answer:  I would expect the proportion of those intervals to capture the true population mean                                 because each bootstrap sample is doen with replacement so you are drawing from the sample                            of 60.
...
```{r iterate}
(sample_props60 <- samp %>%
                    rep_sample_n(size = 60, reps = 1000, replace = TRUE) %>%
                    count(climate_change_affects) %>%
                    mutate(p_hat = n /sum(n)) %>%
                    filter(climate_change_affects == "Yes"))
```
...
```{r }
ggplot(data = sample_props60, aes(x = p_hat)) +
  geom_histogram(binwidth = 0.02) +
  labs(
    x = "p_hat (Climate Change Affects)",
    title = "Sampling distribution of p_hat",
    subtitle = "Sample size = 60, Number of samples = 1000"
  )
```
...
1.  Choose a different confidence level than 95%. Would you expect a confidence 
    ...
    Answeer: I would think that the higher the confidence level the wider the confidence interval would be and the                lower the more narrow the confidence interval.
    
```{r confidence interval infer 99}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
...
```{r confidence interval infer 90}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
