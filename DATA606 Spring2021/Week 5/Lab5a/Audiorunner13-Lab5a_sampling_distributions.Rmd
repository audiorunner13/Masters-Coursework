---
title: "Lab 5a - Foundations for statistical inference - Sampling distributions"
author: "Peter Gatica"
date: "03/14/2021"
output:
  html_document:
...
1.  Would you expect the sample proportion to match the sample proportion of 
    ...
    Answer:  I would expect it to be not close to the original sample because of the sample size being only fifty               or 0.0005 percent of the population and not close to the population proportion of 20%. 
...
1.  Take a second sample, also of size 50, and call it `samp2`. How does the 
    ...
    Answer:  As expected sampl2 proportion was much different than the original with a samp2_phat of 0.16.  As                  expected, the larger the sample size the closer the sample proportion gets to the population                       proportion of 20%
...
```{r inline-calc-samp2, include=TRUE}
# For use inline below
(samp2_p_hat <- samp2 %>% 
  count(scientist_work) %>% 
  mutate(p_hat = n /sum(n)) %>% 
...
```{r inline-calc-samp3, include=TRUE}
# For use inline below
(samp3_p_hat <- samp3 %>% 
  count(scientist_work) %>% 
  mutate(p_hat = n /sum(n)) %>% 
...
```{r inline-calc-samp4, include=TRUE}
# For use inline below
(samp4_p_hat <- samp4 %>% 
  count(scientist_work) %>% 
  mutate(p_hat = n /sum(n)) %>% 
...
1.  How many elements are there in `sample_props50`?  Describe the sampling 
    ...
    Answer:  There are 15,000 samplings of size 50 with replacement in the sampling distribution.  The center of                the distribution is .2, same as the population proportion of 20%.  The spread or standard error of                 .05.  The shape of the sample distribution is symmetric and bell-shaped that it resembles a normal                 distribution.
...
1.  To make sure you understand how sampling distributions are built, and exactly
    ...
    Answer:  There are 25 observations (samples) in sample_props_small.  Each observation represents sample proportion of "Doesn't benefit" responses.
...
1.  Use the app below to create sampling distributions of proportions of 
    *Doesn't benefit* from samples of size 10, 50, and 100. Use 5,000 simulations. 
    ...
    Answer: Each observation represents sample proportion of "Doesn't                 benefit" responses.
    
    How does the mean, standard error, and shape of the sampling distribution change as the sample size increases?
    
    Answer: As the sample size increases the mean gets closer to the                  population proportion of 20% and the standard error gets                  smaller and the shape of the sample distribution resembles                more of a normal distribution. 
    ...
    Answer: For each of the sample sizes, increasing the number of                    simulations does not change the sample proportion, the                    standard error nor the shape of the sample distribution.
...
1.  Take a sample of size 15 from the population and calculate the proportion of 
    ...
    Answer: From this first sample, it appears from this simulation that the best point estimate of the population               proportion of the people who think the work scientists do enhance their lives is 100%.  However, if I run             a second sample, I get a sample proportion of 87%.  This seems like a most best estimate of the                      population proportion.
...
```{r inline-calc-samp15, include=TRUE}
# For use inline below
(samp15_p_hat <- samp15 %>% 
  count(scientist_work) %>% 
  mutate(p_hat = n /sum(n)) %>% 
...
1.  Since you have access to the population, simulate the sampling 
    ...
    Answer:  The plot is slightly skewed to the left but the shape is mainly symmetrical and bell shaped but the bins              are discrete which one can tell by the gaps in the bins.  This would mean that it success failure                    condition would be less than 10.  np >= 10 (13.05) and n(1-p) >= 10 (3).
...
1.  Change your sample size from 15 to 150, then compute the sampling 
    ...
    Answer: From the histogram, the true proportion would be that 80% responded that the work scientists do does                 enhance their lives.  Success-failure condition - (150 x .8) = 120 and 150(1 - .8) = 30.  The shape is               symmetrical and bell shaped and resembles a normal distribution.
...
1.  Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    Answer:  The sampling distribution from 2 has a wider spread than 3.  If I'm concerned with making estimates                  that are more often close to the true value, I would prefer a small spread.
