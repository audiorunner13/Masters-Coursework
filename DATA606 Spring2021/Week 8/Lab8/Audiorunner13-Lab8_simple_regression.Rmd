...
1.  What are the dimensions of the dataset?
    ...
    Answer: The dimensions in the dataset are year, ISO_code, countries and region.
...
1.  What type of plot would you use to display the relationship between the personal
    ...
    Answer:  I would use a scatterplot to display the relationship between the pf_score
             and another numerical variable.
...
1.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    Answer:  There is a positive correlation between pf_expression_control and
             pf_score.  As you can see from the positive upward trend of the
             observations.  The correlation is also strong at almost 80%
...
1.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    Answer:  I ran the plot_ss in an r script in RStudio and the smallest Sum of Squares that I
             got was 952.153.
...
1.  If someone saw the least squares regression line and not the actual data, how
    ...
    Answer: Estimate where 6.7 lies and then mark where the line intersects the 
            x and y axis. This is your predicted value or 8.  I would think that
            this is overestimated. The residual would be the difference between
            the observed residual and the predicted residual.
...
1.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    Answer:  There is an apparent pattern in the residual plot where one can see
             that both positive and negative residuals are tightly packed around
             the least of squares line which suggests a high correlation score
             which suggests a strong correlation to a positive linear trend.
...
1.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    Answer:  Yes it does.  There are not noticeable outliers that are influential
              points to affect the linearity of the observations.
...
1.  Based on the residuals vs. fitted plot, does the constant variability condition 
    ...
    Answer:  Yes, constant variability is met because there is no fanning out of
             the observations.
...
-   Choose another freedom variable and a variable you think would strongly
    ...
    Answer:  I have chosen pf_religion.  There seems to be a postive linear relationship upwards concentrated 
             on the right side of the graph and you would think this to be an obvious relationship since persons
             pf_score would be higher the more religious freedom a person has.  However, the correlation which 
             measures how good the model.  In this case it is mediocre at 49.7% , I expected higher.
...
-   How does this relationship compare to the relationship between 
    ...
    Anser:  The pf_expression_control to pf_score has a much more positive linear relationship
            than the model of pf_religion to pf_score.  It surprises me becuase I would think that 
            having more religious freedom would translate to a stronger linear relatonship but the
            R2 seems to not indicate that with an R2 of 49.7%.
