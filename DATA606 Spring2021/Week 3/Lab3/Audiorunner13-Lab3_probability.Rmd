...
set.seed(35797) 
shot_outcomes <- c("H", "M")
sim_basket_1 <- sample(shot_outcomes, size = 133, replace = TRUE, prob = c(.45, .55))
sim_basket_1
table(sim_basket_1)
```
...
1.  Using `calc_streak`, compute the streak lengths of `sim_basket`, and
    ...
    ```{r calc-streak-sim-1}
sim_streak_1 <- calc_streak(sim_basket_1)
```

We can then take a look at the distribution of these streak lengths.

```{r plot-streak-sim-1}
ggplot(data = sim_streak_1, aes(x = length)) +
  geom_bar()
```
...
1.  Describe the distribution of streak lengths. What is the typical streak 
    ...
    to include a plot in your answer.
      ...
      Answer:  Although the distribution at first appears to be very similar to Kobe's especially the first streak hits are typically very high, at closer review the simulation streak lengths are shorter and there are more streaks than Kobe.  
...
1.  If you were to run the simulation of the independent shooter a second time, 
    ...
    different? Explain your reasoning.
          ...
          Answer: I would you expect its streak distribution to be very similar because since it appears that shots are independent of each other and the probability to hit or miss is the same for each simulation.  Yoou can see that the bar plots look almost alike for both simulations.

```{r sim-basket-2}
shot_outcomes <- c("H", "M")
sim_basket_2 <- sample(shot_outcomes, size = 133, replace = TRUE, prob = c(.45, .55))
sim_basket_2
table(sim_basket_2)
```
...
1.  How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    Answer:  Although Kobe has fewer streaks than the simulated shooter, his streaks are longer.  I would not say that the hot hand model fits Kobe's shooting pattern but what i will say is that Kobe's seems to be more of a consistent 45% shooter than the sim shooter because he streaks are longer with higher made baskets.
