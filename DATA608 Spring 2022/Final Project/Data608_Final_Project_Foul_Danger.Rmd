---
title: "DATA608-Knowledge and Visual Analytics Final Project"
author: "Peter Gatica"
date: "5/18/2022"
output:
  html_document:
...
# The number of foul balls per stadium on its heaviest foul ball day of the 2019 season up until June 5, 2019.
```{r}
...
# The number of foul balls per foul type per Stadium.

```{r}
ui_1 <- fluidPage(
  # titlePanel(title = h4("Foul Balls By Stadium", align="center")),
  
  # imageOutput("image1"),
  
  sidebarPanel(
    
    radioButtons("Stadium", "Foul Balls By MLB Stadium",
                 choices = c("Yankees", "Mariners","Orioles","Pirates","A's","Rangers","Dodgers","Brewers","Phillies","Braves"),
                 selected = "Yankees")),
  
  mainPanel(
    plotOutput("bar",height = 350))
)
```
```{r}
server_1 <- function(input,output){
  ...
  output$bar <- renderPlot({
        ...
        our_data <- ( df_out <- foul_ball_danger[foul_ball_danger$Stadium == input$Stadium ,names(foul_ball_danger) <- c("Stadium","FoulType","ExitVelocity","FoulZone")]) #, rownames = TRUE

  p1 <- ggplot(data = our_data[,c("FoulType","FoulZone")]) +
    geom_bar(mapping = aes(x = FoulType, fill = FoulType)) +
    labs(title = "Number of Foul Balls by Foul Type", x = "Foul Type", y = "Number of Foul Balls" )
...
```{r}
fb_zones_img <- "https://raw.githubusercontent.com/audiorunner13/Masters-Coursework/main/DATA608%20Spring%202022/Final%20Project/fb_zones.png"
...
1. Batter hits self, ground and line drive foul balls mostly likely do not make it into Zone 1 and most of 
   Zones2 and 3 seats because they are stopped by the safety net.
2. Fly balls are the most hit in all stadiums and are mostly hit towards Zones 4 and 5 areas.  Although this is
   a lot most are probably hit into the upper level sections or outside the park.
3. Lined and ground foul balls that are hit into exposed parts of Zones 2 and 3 and exposed Zones 4 and 5. These 
   foul balls probably make it into those seats and do travel at a high rate of speed.
...
# The number of foul balls per zone per Stadium.
...
server_2 <- function(input,output){
  ...
  p2 <- ggplot(data = our_data[,c("FoulZone","FoulType")]) +
    geom_bar(mapping = aes(x = FoulZone, fill = FoulZone)) +
    labs(title = "Number of Foul Balls by Foul Zone", x = "Foul Zone", y = "Number of Foul Balls" )

  grid.arrange(p2)
  })
}

shinyApp(ui=ui_1, server=server_2)
...
Since the exit velocity of some foul balls was not captured the average exit velocity of the entire dataset is 
a low 48.91 mph.  In order to get a more accurate average exit velocity, the records with no exit velocity should 
be filtered.
...
After 326 foul balls with an exit velocity of zero have been filtered, a more accurate average exit velocity can be calculated.
...
# The exit velocity of foul balls hit into each zone by stadium.
...
server_3 <- function(input,output){
  ...
  output$bar <- renderPlot({
    ...
    our_data <- ( df_out <- foul_ball_danger_v2[foul_ball_danger_v2$Stadium == input$Stadium ,names(foul_ball_danger_v2) <- c("Stadium","FoulType","ExitVelocity","FoulZone")]) #, rownames = TRUE

  p3 <- ggplot(data = our_data) +
    geom_point(mapping = aes(x = ExitVelocity, y = FoulZone)) +
    facet_wrap(~ FoulType, nrow = 2) +
    labs(title = "Foul Ball Exit Velocity by Zone by Foul Ball Type", x = "Exit Velocity (mph)", y = "Foul Zone" )
...
shinyApp(ui=ui_1, server=server_3)
...
foul_ball_danger_grln45 <- subset(foul_ball_danger_v2,(FoulType == "Ground" | FoulType == "Line" | FoulType == "Fly") 
                                  & (FoulZone == 4 | FoulZone == 5 | FoulZone == 6 | FoulZone == 7))
...
2.  If the protective netting is not extended beyond Zones 2 and 3 then age restrictions should be in place to prevent younger and senior fans, who may be unable to protect themselves, from sitting the lower seating areas where high velocity foul balls are more likely to be hit.
